...
언어별 진입 파일을 쓰려면 기본 설정을 `cmux-base.lua`로 복사하고, 선택한 언어 파일을 최종 `init.lua`로 복사해야 합니다.
...
실제 설정은 `cmux-base.lua`를 기준으로 로드되고, 언어별 파일은 선택 언어를 표시한 뒤 `cmux-base.lua`를 불러오는 wrapper입니다.
