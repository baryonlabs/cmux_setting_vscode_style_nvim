...
- Ctrl+s가 터미널 흐름 제어와 충돌할 수 있으므로 필수 설치가 아닌 docs/ZSH_SETTINGS.md의 선택 설정으로 안내한다.
- docs/zshrc.cmux를 선택 설치 옵션으로 안내하고, claude 입력 후 위/아래 방향키로 이전 claude 명령을 찾는 zsh-history-substring-search 설정을 포함한다.
